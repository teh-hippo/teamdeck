...
/** Snapshot presence field. `known` is false when the value must render unavailable. */
export type PresenceInfo = { value: Presence; known: boolean };
...
export type TeamsSnapshot = {
	...
	/** Controls whose UIA state cannot be resolved safely. */
	compatibilityIssues?: string[];
	/** Coarse Teams availability, from the opt-in Teams-log reader. Absent from an older helper. */
	presence?: PresenceInfo;
	/** Presence opt-in, injected by the client from the persisted setting. */
	logReadingAllowed?: boolean;
};

export type ToggleStateField = "isMuted" | "isVideoOn" | "isHandRaised";
